...
(
	SELECT t2.customerNumber, t3.customerName, t3.city, t3.country,
	...
	WHERE YEAR(t2.orderDate) IN (2003, 2004) AND t2.status = 'Shipped'
	GROUP BY year, t2.customerNumber, t3.customerName, t3.city, t3.country
), 

		
		previous_revenue_cte AS
		(
		SELECT customerNumber, customerName, city, country,  year, total_orders, customer_revenue,
		LAG (customer_revenue) OVER (PARTITION BY customerNumber ORDER BY year) AS previous_revenue
		FROM top_customers
		ORDER BY customerNumber, year
            ...
            (
			SELECT *,
            ROUND((customer_revenue - previous_revenue) / previous_revenue * 100, 2) AS YoY_growth
